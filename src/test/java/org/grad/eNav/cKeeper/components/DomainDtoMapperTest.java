...
class DomainDtoMapperTest {
    ...
    /**
     * Test that we can correctly convert sets between domain and DTO objects.
     */
    @Test
    void testConvertToSet() {
        Set<TestDtoClass> result = this.domainDtoMapper.convertToSet(new HashSet<>(this.testDomainClassList), TestDtoClass.class);
        assertNotNull(result);
        assertEquals(1, result.size());
        assertEquals(this.testDomainClassList.get(0).getI(), result.stream().findFirst().map(TestDtoClass::getI).orElse(null));
        assertEquals(this.testDomainClassList.get(0).getString(),result.stream().findFirst().map(TestDtoClass::getString).orElse(null));
        assertTrue(Arrays.equals(this.testDomainClassList.get(0).getBytes(), result.stream().findFirst().map(TestDtoClass::getBytes).orElse(null)));
        assertTrue(Arrays.equals(this.testDomainClassList.get(0).getList().toArray(), result.stream().findFirst().map(TestDtoClass::getList).map(List::toArray).orElse(null)));
    }
